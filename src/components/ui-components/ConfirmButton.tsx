...
export class ConfirmButton extends React.PureComponent<LoginButtonProps> {
  render() {
    const { type = "submit", loading, className, text, disabled = false, onCLick, wrapperClassName } = this.props;
    return(
      <FlexBox className={ wrapperClassName } justifyContent={ JustifyContent.center }>
        {
          !loading ? (
            <button
